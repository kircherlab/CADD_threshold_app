...
def compare_and_update_panel_data(old_file_path, new_file_path):
    ...
    backup_file_path = os.path.join(backup_dir, f"panels_summary_{datetime.now().strftime('%Y-%m-%d')}_backup.csv")
    ...
    for panel_id in df_new_idx.index:
        ...
        if panel_id in df_old_idx.index:
            ...
            if str(new_row.get('Version')) != str(old_row.get('Version')):
                print(f"Version changed for Panel {panel_id}: {old_row.get('Version')} -> {new_row.get('Version')}")
                info = get_panel_info(panel_id)
                if info is not None:
                    df_old_idx.at[panel_id, 'Version'] = info.get('Version', new_row.get('Version'))
                    df_old_idx.at[panel_id, 'Genes'] = ";".join(info.get('Genes', []))
                    df_old_idx.at[panel_id, 'GeneCount'] = info.get('GeneCount', new_row.get('GeneCount', 0))
                    df_old_idx.at[panel_id, 'DateOfCheck'] = datetime.now().strftime('%Y-%m-%d')
                else:
                    print(f"Warning: failed to fetch panel info for {panel_id}; leaving entry unchanged")
            else:
                # same version -> update date of check
                df_old_idx.at[panel_id, 'DateOfCheck'] = datetime.now().strftime('%Y-%m-%d')
        else:
            # new panel: fetch info and add
            print(f"New panel found: {panel_id} (version {new_row.get('Version')}); fetching info")
            info = get_panel_info(panel_id)
            if info is not None:
                ...
                additional_rows.append({
                    ...
                    'Genes': fallback_genes if isinstance(fallback_genes, str) else ";".join(fallback_genes),
...
compare_and_update_panel_data('data/paneldata/panels_summary_2025-11-20.csv', 'data/paneldata/panels_and_versions_summary.csv')

'''
def _parse_args_and_run():
    parser = argparse.ArgumentParser(description='Compare two panel CSVs and update the old one with new/changed entries')
    parser.add_argument('old_csv', nargs='?', default='./data/paneldata/panels_summary.csv', help='Path to old panels_summary CSV')
    parser.add_argument('new_csv', nargs='?', default='./data/paneldata/panels_and_versions_summary.csv', help='Path to new panels+versions CSV')
    args = parser.parse_args()
    compare_and_update_panel_data(args.old_csv, args.new_csv)
