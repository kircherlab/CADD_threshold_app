...
def main_panelapp():
    fetch_all_panels_and_versions(
        pages=5,
        save_csv=True,
        csv_path=str(APP_ROOT / "data" / "paneldata" / "panels_and_versions_summary.csv"),
    )
    compare_and_update_panel_data(
        str(APP_ROOT / "data" / "paneldata" / "panels_summary_2025-11-20.csv"),
        str(APP_ROOT / "data" / "paneldata" / "panels_and_versions_summary.csv"),
    )
