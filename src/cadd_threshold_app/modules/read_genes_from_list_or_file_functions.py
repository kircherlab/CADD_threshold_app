...
def read_genes_from_list_input(text):
    ...
    cleaned = [g.strip().strip("[]'\"").upper() for g in genes if g and g.strip()]
    return cleaned
...
def read_df_or_lines(file_path: str, sep):
    ...
    try:
        if sep:
            df = pd.read_csv(
                file_path, delimiter=sep, header=None, engine="python", dtype=str
            )
        else:
            df = pd.read_csv(
                file_path, sep=None, header=None, engine="python", dtype=str
            )
